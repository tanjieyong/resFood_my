...
insert into resadmin( raname, rapwd) values( 'administrator', 'd7afde3e7059cd0a0fe09eec4b0008cd');
insert into resadmin( raname, rapwd) values( 'b', 'd7afde3e7059cd0a0fe09eec4b0008cd');
...
insert into resuser( username, pwd,email) values( 'admin', 'd7afde3e7059cd0a0fe09eec4b0008cd','a@163.com');
...
select * from (select resuser.userid,username,ifnull( sum(dealprice*num),0) as dealcount  from resuser 
left join resorder on resuser.userid=resorder.userid  
left join resorderitem on resorder.roid=resorderitem.roid  group by userid,username ) a  
order by dealcount desc  limit 0,100
